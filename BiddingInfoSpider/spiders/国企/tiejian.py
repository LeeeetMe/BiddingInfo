...
class TieJianSB(BaseSpider):
    ...
    start_urls = ['http://ece.crcc.cn/homepage/bidList.jhtml?page=1']
    website_name = '铁建设备'
    tmpl_url = [
        'http://ece.crcc.cn/homepage/queryInviteList.jhtml?page=1&invitename=&invitecode=&beginTime=&endTime=&start=%s&limit=20' % i for i in range(0, 100,20)]
...
class TieJianWZ(BaseSpider):
    name = 'tiejianwz'
    allowed_domains = ['ece.crcc.cn']
    start_urls = ['http://ecm.crcc.cn/unlogin/queryInit.jhtml?model=1&type=1&search=']
    website_name = '铁建物资'
    tmpl_url = [
        'http://ecm.crcc.cn/unlogin/queryPurchase.jhtml?model=1&title=&code=&pubstartdate=&pubenddate=&orgname=&start=%s&limit=20' % i for i in range(0, 100,20)]
